...
//将内核事件表注册读事件，ET模式（EPOLLET边缘触发模式），选择开启EPOLLONESHOT（保证一个socket只能同时被一个线程（进程）操作）
...
void http_conn::init()
{
    ...
    memset(m_read_buf, '\0', READ_BUFFER_SIZE); //memset是一个初始化函数，作用是将某一块内存中的全部设置为指定的值。
...
bool http_conn::read_once()
{
    ...
    {
        // 接收数据，保存到读缓存区
        bytes_read = recv(m_sockfd, m_read_buf + m_read_idx, READ_BUFFER_SIZE - m_read_idx, 0); //359
        // 修改m_read_idx的读取字节数
...
http_conn::HTTP_CODE http_conn::do_request()
{
    ...
    else if (*(p + 1) == '1')
    {
        ...
        strncpy(m_real_file + len, m_url_real, strlen(m_url_real)); //字符串限长复制
    ...
    //stat()用来将参数m_real_file所指的文件状态, 复制到参数&m_file_stat所指的结构中。
