                     ...
                     int opt_linger, int trigmode, int sql_num, int thread_num, int close_log, int actor_model)
{
    m_port = port; //9006
    m_user = user; //root
    m_passWord = passWord; //"501501"
    m_databaseName = databaseName; // "wbdb"
    m_sql_num = sql_num; //8
    m_thread_num = thread_num; //8
    m_log_write = log_write; //0
    m_OPT_LINGER = opt_linger; //0
    m_TRIGMode = trigmode; //0
    m_close_log = close_log; //0
    m_actormodel = actor_model; //0
}
...
{
    //网络编程基础步骤
    m_listenfd = socket(PF_INET, SOCK_STREAM, 0); //12
    //如果它的条件返回错误，则终止程序执行
    assert(m_listenfd >= 0);
...
bool WebServer::dealclinetdata()
{
    ...
    socklen_t client_addrlength = sizeof(client_address); //16
    //LT模式只要输入缓冲有数据就会一直通知该事件
    if (0 == m_LISTENTrigmode)
    {
        int connfd = accept(m_listenfd, (struct sockaddr *)&client_address, &client_addrlength); //16
        if (connfd < 0)
        {
