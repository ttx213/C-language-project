            ...
            if(frame) {
                ...
                if(( (frame->format != audio_output->dst_tgt_.fmt)      // 采样格式的判断
                   ...
                   && (!audio_output->swr_ctx_)) {
                    /*
                    &audio_output->swr_ctx_：指向 SwrContext 结构体指针的指针，用于接收配置后的音频重采样器对象。
                    &audio_output->dst_tgt_.ch_layout：音频重采样后的目标通道布局。
                    audio_output->dst_tgt_.fmt：音频重采样后的目标样本格式。
                    audio_output->dst_tgt_.freq：音频重采样后的目标采样频率。
                    &frame->ch_layout：输入音频帧的通道布局。
                    (enum AVSampleFormat)frame->format：输入音频帧的样本格式。
                    frame->sample_rate：输入音频帧的采样频率。
                    0：重采样过程中的额外选项，这里设置为0表示没有额外选项。
                    NULL：重采样过程中的额外选项参数，这里设置为NULL表示没有额外选项参数。
                    */
                ...
                if(audio_output->swr_ctx_) {
                    ...
                    //将分配的内存块的地址存储到 audio_output->audio_buf1_ 指向的变量中，并将实际分配的内存块的大小存储到 audio_output->audio_buf1_size 指向的变量中。
                    av_fast_malloc(&audio_output->audio_buf1_, &audio_output->audio_buf1_size, out_bytes);
                    //将输入音频数据进行重采样，并将结果写入到输出音频数据缓冲区中。参数len2返回实际进行重采样的样本数。
